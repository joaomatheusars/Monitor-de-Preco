...
class PopFrame(customtkinter.CTkFrame):
    ...
    def mensage(self, text):
        ...
        self.grid(row=1, column=0, padx=8, pady=8, sticky="ewn", ipadx=8, ipady=8)
...
class SearchFrame(customtkinter.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.btn_adicionar = self.buttons(text='Adicionar', bg_color='#3ab42f', hover_color='#49E13B', command=self.action_adicionar)
        self.btn_adicionar.grid(row=1, column=0, padx=8, stick='nesw')
        
        self.btn_monitorar = self.buttons(text='Monitorar', bg_color='#22619c', hover_color='#2B7AC3', command=self.action_monitorar)
    ...
    def buttons(self, text='', bg_color='blue', hover_color='blue', size=16, command: Union[Callable[[], Any], None] = None):
        ...
        button = customtkinter.CTkButton(self,text=text, fg_color=bg_color, hover_color=hover_color,  font=my_font, command=command)
        return button
    
    def action_adicionar(self):
        if function.verify_link(self.input_link.get()):
            try:
                link = threading.Thread(target=Kabum, args=(self.input_link.get(),)).start()
    ...
    def monitor(self):
        try:     
            ...
            self.popframe.grid_remove()
            self.btn_stop_monitorar = self.buttons(text='Para Monitoramento', bg_color='#F12C2C', hover_color='#EE3C3C', command=self.stop_monitorar)
            ...
            while True:   
                ...
                if self.cancel_monitor():break
        except:
            self.popframe.mensage("Ainda não existe nenhum produto para monitorar")
            
    def stop_monitorar(self):
        self.sig = False
        self.btn_stop_monitorar.configure(text='Finalizando...', state="disabled")        
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.top_frame.grid(row=0, column=0, padx=8, pady=8, sticky="ewns", ipadx=8, ipady=8)
